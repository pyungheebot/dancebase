...
export default function StatsPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    if (data) {
      const groupStats: GroupStats[] = data.map((row: { group_id: string; group_name: string; total_schedules: number; present_count: number; late_count: number; absent_count: number; early_leave_count: number }) => {
        ...
        const rate = total > 0 ? Math.round(((present + late + earlyLeave) / total) * 100) : 0;
        return {
  ...
  const overallEarlyLeave = stats.reduce((sum, s) => sum + s.early_leave_count, 0);
  const overallTotal = stats.reduce((sum, s) => sum + s.total_schedules, 0);
  const overallRate =
    overallTotal > 0
      ? Math.round(((overallPresent + overallLate + overallEarlyLeave) / overallTotal) * 100)
      : 0;
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4 text-xs border rounded px-3 py-2">
              <span className="flex items-center gap-1">
                ...
                전체 <span className="font-semibold tabular-nums">{overallRate}%</span>
              </span>
              <span className="flex items-center gap-1">
                <CheckCircle className="h-3 w-3 text-green-500" />
                출석 <span className="font-semibold tabular-nums">{overallPresent}</span>
              </span>
              <span className="flex items-center gap-1">
                <Clock className="h-3 w-3 text-yellow-500" />
                지각 <span className="font-semibold tabular-nums">{overallLate}</span>
              </span>
              <span className="flex items-center gap-1">
                조퇴 <span className="font-semibold tabular-nums">{overallEarlyLeave}</span>
              </span>
              <span className="flex items-center gap-1">
                <XCircle className="h-3 w-3 text-red-500" />
                결석 <span className="font-semibold tabular-nums">{overallTotal - overallPresent - overallLate - overallEarlyLeave}</span>
              </span>
            </div>

            {/* 그룹별 통계 */}
            <div>
              <h2 className="text-xs font-medium text-muted-foreground mb-2">그룹별</h2>
                ...
                {stats.map((stat) => (
                  <div key={stat.group_id} className="rounded border px-3 py-2">
                    ...
                    <div className="flex gap-4 text-[11px] text-muted-foreground mb-1.5">
                      ...
                      <span className="text-orange-600">조퇴 {stat.early_leave_count}</span>
